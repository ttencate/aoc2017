  ...
  (let [next-step (find-next-step lines (:pos cur-state) (:step cur-state))]
    (if (nil? next-step)
      ...
      (let [next-pos (plus (:pos cur-state) next-step)]
        (let [next-letters (append-letters (:letters cur-state) (char-at lines next-pos))]
          (->State next-pos next-step next-letters))))))
(defn walk [lines]
...
(let [lines (string/split-lines (slurp *in*))]
  (let [end-state (last (walk lines))]
    (println (string/join (:letters end-state)))))
